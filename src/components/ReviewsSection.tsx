  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl w-[95vw] max-h-[90vh] bg-[#141414] border-gray-800 text-white p-0 flex flex-col rounded-lg shadow-2xl overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-gray-800/50 bg-[#1a1a1a] relative">
          <div className="flex items-center justify-center relative">
            ...
            <button
              ...
              className="absolute right-0 rounded-sm opacity-70 hover:opacity-100 transition-opacity"
            >
              <X className="h-5 w-5" />
        ...
        <div className="px-6 pt-6 pb-4 border-b border-gray-800/50 bg-[#1a1a1a] flex justify-center items-center flex-col">
          <div className="text-xs text-gray-400 mb-2">Overall rating</div>
          <div className="flex items-baseline gap-2 mb-2">
            <span className="text-4xl font-bold text-white">{overallRating.toFixed(2)}</span>
            <StarIcon size={24} />
          </div>
          <div className="text-sm text-gray-400">1872 ratings & reviews</div>
        </div>

        {/* Filter Chips */}
        <div className="px-6 py-4 border-b border-gray-800/50 bg-[#1a1a1a] flex flex-wrap gap-2">
              ...
              currentReviews.map((review, index) => (
                <li key={review.id} className="flex min-w-0 flex-col">
                  ...
                  {index < currentReviews.length - 1 && (
                    <div className="mb-6 h-px bg-gray-800/50 w-full" />
                  )}
                </li>
              ))
            ) : (
              <li className="text-center text-gray-400 py-8">No reviews found for this filter.</li>
            )}
          </ul>
        </div>
        ...
        {/* Pagination Footer */}
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-gray-800/50 bg-[#1a1a1a] flex items-center justify-center gap-4">
            <button
              onClick={handlePrevious}
              disabled={currentPage === 1}
              className="text-sm text-gray-400 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-1"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[0.38fr_0.62fr] gap-12 lg:gap-20">
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-baseline gap-2">
                <div className="flex items-center gap-0.5" aria-label={`${overallRating.toFixed(2)} out of 5 stars`}>
                  {[...Array(5)].map((_, i) => (
            ...
            <div className="flex w-full min-w-0 flex-col">
              {isLoading ? (
                ...
                <div className="text-gray-400 py-8">No reviews available</div>
              ) : (
                <ul className="flex flex-col">
